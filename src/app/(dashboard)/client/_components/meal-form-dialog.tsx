...
const MealFormDialog = ({ smallTrigger, session }: MealFormDialogProps) => {
  ...
  // Set the form date to match the current filter date when opening for create
  useEffect(() => {
    if (mealDialogOpen && !selectedMealId && mealFilters.dateTime) {
      form.setValue("dateTime", mealFilters.dateTime);
    }
  }, [mealDialogOpen, selectedMealId, mealFilters.dateTime, form]);
  ...
  const handleDialogOpenChange = (open: boolean) => {
    ...
    if (!open) {
      ...
      // Reset to filter date when opening dialog again
      if (mealFilters.dateTime) {
        form.setValue("dateTime", mealFilters.dateTime);
      }
    }
  };
